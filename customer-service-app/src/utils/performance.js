...
export class MemoryManager {
   ...
   */
  triggerCleanup() {
    logger.warn(`Memory usage high: ${(performance.memory.usedJSHeapSize / 1048576).toFixed(2)}MB`);
...
export const measureExecutionTime = (fn, name = 'Function') => {
  return function(...args) {
    ...
    logger.debug(`[Performance] ${name} execution time: ${(end - start).toFixed(2)}ms`);
...
export const measureAsyncExecutionTime = (fn, name = 'AsyncFunction') => {
  return async function(...args) {
    ...
    try {
      ...
      const end = performance.now();
      logger.debug(`[Performance] ${name} failed after: ${(end - start).toFixed(2)}ms`);
      throw error;
    }
  };
};