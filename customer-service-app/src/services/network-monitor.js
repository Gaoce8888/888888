...
export class NetworkMonitor extends EventEmitter {
   ...
   */
  handleOnline() {
    logger.debug('[NetworkMonitor] Network online');
    this.isOnline = true;
   ...
   */
  handleOffline() {
    logger.debug('[NetworkMonitor] Network offline');
    this.isOnline = false;
  ...
  handleConnectionChange() {
    ...
    logger.debug('[NetworkMonitor] Network type changed:', this.networkType);
  ...
  assessNetworkQuality() {
    ...
    if (Math.abs(oldQuality - quality) > 10) {
      ...
      logger.debug(`[NetworkMonitor] Network quality changed: ${quality}%`);
    }
  ...
  async testBandwidth() {
    ...
    try {
      ...
      logger.error('[NetworkMonitor] Bandwidth test failed:', error);
      return 0;
    }
  }
