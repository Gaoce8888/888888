...
export class MessageCache {
  ...
  async initStorage() {
    try {
      ...
      logger.error('[MessageCache] Storage init failed:', error);
    }
  }
  ...
  async restoreFromStorage() {
    ...
    try {
      ...
      logger.debug(`[MessageCache] Restored ${this.cache.size} items from storage`);
    } catch (error) {
      logger.error('[MessageCache] Failed to restore from storage:', error);
    }
  }
  ...
  async set(key, value, ttl = this.options.ttl) {
    ...
    if (this.storage && this.options.persistEnabled) {
      try {
        ...
        logger.error('[MessageCache] Failed to persist item:', error);
      }
    }
    ...
    if (item) {
      ...
      if (this.storage) {
        try {
          ...
          logger.error('[MessageCache] Failed to delete from storage:', error);
        }
      }
  ...
  async clear() {
    ...
    if (this.storage) {
      try {
        ...
        logger.error('[MessageCache] Failed to clear storage:', error);
      }
    }
  }
  ...
  compress(data) {
    try {
      ...
      logger.error('[MessageCache] Compression failed:', error);
      return data;
    }
  }
  ...
  decompress(data) {
    try {
      ...
      logger.error('[MessageCache] Decompression failed:', error);
      return data;
    }
  }
  ...
  async cleanup() {
    ...
    if (expiredKeys.length > 0) {
      logger.debug(`[MessageCache] Cleaned up ${expiredKeys.length} expired items`);
    }
  }
  ...
  async warmup(keys, loader) {
      ...
      if (!this.cache.has(key)) {
        promises.push(
          loader(key).then(data => {
            ...
            logger.error(`[MessageCache] Failed to warmup ${key}:`, error);
          })
        );
      }
    }
    ...
    logger.debug(`[MessageCache] Warmed up ${promises.length} items`);
  }
