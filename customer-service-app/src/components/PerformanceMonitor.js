...
export const PerformanceMonitor = ({ 
  ...
  const updateMetrics = (renderTime) => {
    ...
    if (renderTime > threshold) {
      logger.warn(`[PerformanceMonitor] ${name} render time (${renderTime.toFixed(2)}ms) exceeded threshold (${threshold}ms)`);
    }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (measures.length > 0) {
        ...
        logger.debug(`[PerformanceMonitor] ${name} total time: ${measure.duration.toFixed(2)}ms`);
      }
    };
  }, [name]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.debug(`[Performance] ${componentName} - Render #${renderCountRef.current}: ${renderTime.toFixed(2)}ms (avg: ${avgRenderTime.toFixed(2)}ms)`);
      }
    };
  });
