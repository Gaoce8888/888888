    ...
    const intervalId = setInterval(() => {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        Object.entries(report).forEach(([name, metrics]) => {
          ...
          console.log('Avg Render Time:', `${metrics.renderMetrics.avgDuration.toFixed(2)}ms`);
          console.table(metrics.actions);
          console.groupEnd();
        });
        console.groupEnd();
...
export const useMemoryUsage = (interval = 5000) => {
  ...
  useEffect(() => {
    ...
    const updateMemory = () => {
      ...
      // 内存警告
      if (usage > 90) {
        console.warn(`[Memory] High memory usage: ${usage.toFixed(2)}%`);
      }
    };
  ...
  useEffect(() => {
    ...
    const measureFPS = () => {
      ...
      if (delta >= 1000) {
        ...
        // FPS警告
        if (currentFPS < 30) {
          console.warn(`[FPS] Low frame rate: ${currentFPS} FPS`);
        }
      }
